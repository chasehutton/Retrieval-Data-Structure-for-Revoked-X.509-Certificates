...
//     // Define a Bernoulli distribution with equal probability (0.5) for 0 and 1
...
class SerializableInterleavedSolution {
 public:
  ...
  Index kFixedNumColumns;

  // Does not take ownership of `data` but uses it (up to `data_len` bytes)
  // throughout lifetime
  SerializableInterleavedSolution(char* data, size_t data_len, Index k)
      : data_(data), data_len_(data_len), kFixedNumColumns(k) {}
  ...
  Index GetUpperNumColumns() const {
    if (kFixedNumColumns > 0) {
      return kFixedNumColumns;
    } else {
  ...
  bool FilterQuery(const Key& input, const FilterQueryHasher& hasher) const {
    ...
    if (TS::kAllowZeroStarts && num_starts_ == 0) {
      ...
      InterleavedPrepareQuery(input, hasher, *this, &hash, &segment_num,
                              &num_columns, &start_bit);
      if (kFixedNumColumns > 0 && kFixedNumColumns <= 4) {
        // Use branchless query
        ResultRow mask = (ResultRow{1} << kFixedNumColumns) - 1;
        ResultRow expected = hasher.GetResultRowFromHash(hash) & mask;
  ...
  double ExpectedFpRate() const {
    ...
    if (TS::kAllowZeroStarts && num_starts_ == 0) {
      // Unusual. Zero starts presumes no keys added -> always false
      return 0.0;
    } else if (kFixedNumColumns > 0) {
      // Fixed number of columns.
      // Each result (solution) bit (column) cuts FP rate in half.
      return std::pow(0.5, kFixedNumColumns);
    } else {
 ...
 protected:
  ...
  void InternalConfigure() {
    ...
    if (kFixedNumColumns > 0) {
      assert(num_blocks * kFixedNumColumns <= num_segments);
      num_segments = num_blocks * kFixedNumColumns;
